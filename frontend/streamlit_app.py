...
if st.button("Predire", type="primary"):
    ...
    try:
        ...
        prediction_value = round(result.get("prediction", 0), 3)
        st.metric("Prediction", prediction_value)
        st.success(f"Prediction : {prediction_value}")
        st.write("Variante utilisee :", result.get("variant", "N/A"))
        st.write("Version modele :", result.get("model_version", "N/A"))
        st.write("Mode d'execution :", result.get("execution_mode", "N/A"))
        st.write("Latence :", result.get("latency_ms", "N/A"), "ms")
        st.write("request_id :", result.get("request_id", "N/A"))
